...
def load_egresos(conn: GSheetsConnection, ttl_s: int = 60) -> pd.DataFrame:
    ...
    for c in ["Egreso_ID", "Fecha", "Concepto", "Categoria", "Monto", "Notas", "Drop"]:
        df[c] = df[c].astype(str).fillna("").str.strip()
    return df
...
if page == "Ventas":
    ...
    if modo_actual == "ventas":
        ...
        existing_cats = []
        if "Categoria" in egresos_df_full.columns and not egresos_df_full.empty:
            tmp = (
                egresos_df_full["Categoria"]
                .astype(str)
                .fillna("")
                .str.strip()
                .replace("nan", "")
            ...
            existing_cats = sorted({c for c in tmp if c})
        ...
        if st.button("+ Registrar Gasto", use_container_width=True, disabled=not can_save_eg, key="eg_guardar_btn"):
            ...
            st.success(f"Egreso guardado: {new_id}")
            ss["eg_monto"] = 0.0
            ss["eg_concepto"] = ""
            ss["eg_notas"] = ""
            ss["eg_drop_sel"] = ""
        ...
        with st.expander("Más detalles", expanded=False):
            ...
            with cat_cols[0]:
                cat_opts = ["(Sin categoría)"] + existing_cats
            ...
            with cat_cols[1]:
                ...
                if st.button("➕", key="eg_cat_add_btn", use_container_width=True, help="Agregar y seleccionar"):
                    v = (ss["eg_categoria_new"] or "").strip()
                    if v:
                        ss["eg_categoria_sel"] = v
                        ss["eg_categoria_new"] = ""
                        st.rerun()
